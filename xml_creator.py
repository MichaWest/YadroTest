...
class XML_creator:
    ...
    def create_file(self):
        ...
        root_tag = self.__get_root_tag()
        root_node = self.__create_structure(root_tag)
        self.__write_output(root_node)

        #self.__write_output__(self.__create_structure__(self.__get_root_tag__()))


    def __write_output(self, root_node: XML_node):
        root_tag = self.__create_output(root_node)
    ...
    '''
    def __create_output(self, root_node: XML_node):
        root_tag = ET.Element(root_node.name)
        self.__create_tag_param(root_tag, root_node)
        kids = root_node.children
        for k in kids:
            self.__create_subelement(root_tag, k)

        return root_tag

    def __create_tag_param(self, tag, node):
        for p in list(node.parameters):
            param_tag = ET.SubElement(tag, p)
            param_tag.text = node.parameters[p]

    def __create_subelement(self, ptag, node: XML_node):
        tag = ET.SubElement(ptag, node.name)
        self.__create_tag_param(tag, node)
        for k in node.children:
            self.__create_subelement(tag, k)


    '''
    Определяем дерево элементов (XMl_Node)
    '''
    def __create_structure(self, tag) -> XML_node:
        tag_attrs = self.xml_reader.get_attribute(tag)
        tag_name = tag_attrs['name']
        xml_node = XML_node(tag_name)

        xml_node = self.__write_parameters(tag, xml_node)
        ...
        for t in list(source_dict):
            source_node = self.__create_structure(t)
            xml_node.add_child(source_node)

        return xml_node

    def __write_parameters(self, tag, xml_node: XML_node) -> XML_node:
